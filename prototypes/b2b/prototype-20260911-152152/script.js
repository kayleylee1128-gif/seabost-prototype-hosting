...
const wholesaleValues=[{seq:1,platform:'Amazon',store:'全部店铺',sku_qty:50,order_total_qty:100,auto_purchase:'是',shortage_rate:'20%',status:'启用',logs:[{operator:'林晓',action:'新增数据',time:'2026-09-02 14:35:20',detail:'创建批发单定义案例数据'}]}];
...
function openRowLogs(index){const item=values[index],logs=item.logs||[];$('#rowLogSubtitle').textContent=`序号 ${item.seq} · ${item[fields[0].code]??'—'}`;$('#rowLogList').innerHTML=logs.length?logs.map(log=>`<p><b>${log.operator}</b> ${log.action}<small>${log.time}</small><span>${log.detail}</span></p>`).join(''):'<div class="c-empty">当前行暂无变更日志</div>';openDrawer('#rowLogDrawer')}
...
function buildImportExample(){const headers=['序号',...fields.map(field=>field.code)];const first=values[0];const update=['1',...fields.map(field=>first?.[field.code]??field.default??'')];const add=['',...fields.map((field,index)=>index===0?'新增示例':field.type==='数字'?'1':field.type==='布尔'?'是':'示例值')];$('#importText').value=[headers.join(','),update.join(','),add.join(',')].join('\n');$('#importHint').textContent=`字段：${headers.join('、')}。序号 1 为更新示例，空序号为新增示例。`;$('#importPreview').innerHTML=''}
function parseImport(){const lines=$('#importText').value.trim().split(/\r?\n/).filter(Boolean);if(lines.length<2)throw new Error('至少需要表头和一行数据');const headers=lines[0].split(',').map(v=>v.trim());const expected=['序号',...fields.map(field=>field.code)];if(headers.join('|')!==expected.join('|'))throw new Error(`表头必须为：${expected.join(',')}`);return lines.slice(1).map((line,rowIndex)=>{const cells=line.split(',').map(v=>v.trim());if(cells.length!==headers.length)throw new Error(`第 ${rowIndex+2} 行字段数量不正确`);return Object.fromEntries(headers.map((header,index)=>[header,cells[index]]))})}
function importValues(){let rows;try{rows=parseImport()}catch(error){toast(error.message);return}let added=0,updated=0;rows.forEach(row=>{const importedSeq=Number(row.序号),existingIndex=importedSeq?values.findIndex(item=>Number(item.seq)===importedSeq):-1;const payload=Object.fromEntries(fields.map(field=>[field.code,row[field.code]]));if(existingIndex>=0){const item=values[existingIndex];Object.assign(item,payload);addRowLog(item,'批量导入更新',`通过序号 ${item.seq} 更新数据`);updated++}else{const seq=nextSeq(),item={seq,...payload,status:'启用',logs:[]};addRowLog(item,'批量导入新增',row.序号?`导入序号 ${row.序号} 不存在，系统生成新序号 ${seq}`:`未提供序号，系统生成新序号 ${seq}`);values.push(item);added++}});markUpdated(`批量导入配置数据：新增 ${added} 条，更新 ${updated} 条`);closeDrawers();renderValues();toast(`导入完成：新增 ${added} 条，更新 ${updated} 条`)}
...
$('#addFieldBtn').onclick=()=>openFieldEditor();$('#addValueBtn').onclick=()=>openValueEditor();$('#importValueBtn').onclick=()=>{buildImportExample();openDrawer('#importDrawer')};
...
const anns=[
  ...
  {id:9,type:'交互',title:'批量导入',text:'点击“批量导入”打开 CSV 导入抽屉。序号为空或不存在时新增并生成新序号；序号存在时更新对应行，完成后反馈新增与更新数量。',target:'importValueBtn'},
  {id:10,type:'交互',title:'行级日志',text:'每行操作栏提供“日志”；点击后查看该序号的新增、编辑、启停及批量导入记录，包含操作人、时间和变更说明。',target:'rowLogDrawer'}
];
